...
@section('content')
    <main class="dark:bg-dark">
        <div class="container mx-auto px-5 py-28">
            <div class="mb-8 mt-8 flex w-full flex-wrap">
                <div class="mb-6 w-full lg:mb-0">
                    <h1 class="text-2xl font-semibold capitalize text-green-500 md:text-xl lg:text-4xl">
                        {{ $title }} <br> <span
                            class="text-gray-800 underline decoration-blue-500 dark:text-white">Menilai
                            dengan jujur dan
                            baik</span></h1>

                    <p class="mt-4 text-gray-500 dark:text-gray-400 xl:mt-6">
                        Penilaian yang Anda berikan sangat berharga bagi kami
                    </p>
                </div>
            </div>
            {{-- <form action="{{ route('penilaian.index') }}">
                @if (request('category'))
                    <input type="hidden" name="category" value="{{ request('category') }}">
                @endif
                <div class="mx-auto flex max-w-md items-center" x-data="{ search: '' }">
                    <div class="w-full">
                        <input
                            class="focus:shadow-outline mr-10 block w-full appearance-none rounded-l-lg border border-transparent bg-gray-200 py-2 px-4 leading-normal placeholder-gray-700 placeholder:text-slate-400 focus:border-gray-300 focus:bg-white focus:outline-none dark:bg-gray-700 dark:text-white"
                            name="search" value="{{ request('search') }}" type="search"
                            placeholder="Cari deskripsi..." x-model="search">
                    </div>
                    <div>
                        <button type="submit"
            ...
            @if ($topics->count())
                <div
                    ...
                    @foreach ($topics as $topic)
                        <div class="rounded-lg bg-white p-8 dark:bg-slate-800">
                            <div class="mb-3 flex items-center">
                                <div
                                    class="mr-3 inline-flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-full bg-indigo-500 text-white dark:bg-gray-700">
                                    <i class="far fa-value-absolute"></i>

                                </div>
                                <a href="/penilaian?category={{ $topic->category->slug }}"
                                    class="relative flex flex-wrap items-center justify-center text-xs font-bold text-white dark:text-green-400">
                                    <div class="rounded bg-green-500 px-2 py-1 dark:bg-slate-700">
                                        {{ $topic->category->name }}
                                    </div>
                                </a>
                            </div>
                            ...
                            {{-- <span
                                class="my-3 flex flex-row items-center rounded-md bg-indigo-600 py-3 px-3 pr-5 shadow-md dark:bg-indigo-900">
                                <svg class="mr-3 text-white dark:text-white" fill="currentColor"
                                    height="13px" width="13px" version="1.1" id="Layer_1"
                                ...
                                <span id="countdown {{ $topic->id }}"
                                    class="ml-1 text-white dark:text-white">
                                    <script>
                                        CountDownTimer('{{ $topic->start_date }}', 'countdown {{ $topic->id }}');

                                        function CountDownTimer(dt, id) {
                                            var end = new Date('{{ $topic->end_date }}');
                                            var timer;
                                            ...
                                            function showRemaining() {
                                                ...
                                                if (distance < 0) {
                                                    ...
                                                    clearInterval(timer);
                                                    document.getElementById(id).innerHTML = 'Penilaian Ditutup';
                                                    return;
                                                }
                                                ...
                                                document.getElementById(id).innerHTML =
                                                    '<h4 class="mr-3 text-slate-600 text-white dark:text-white">Penilaian Dibuka Selama</h4>';
                            ...
                            <div>
                                <div class="font-medium dark:text-slate-400">
                                    <p class="my-5 text-base leading-relaxed">
                                        {!! $topic->description !!}</p>
                                </div>
                                <hr class="mt-6 w-full" />
                                {{-- @if (\Carbon\Carbon::now()->between(\Carbon\Carbon::parse($topic->start_date), \Carbon\Carbon::parse($topic->end_date)))
                                    @if ($topic->evaluation()->where(['user_id' => Auth::user()->id, 'topic_id' => $topic->id])->first()) --}}
                                        {{-- <div class="mt-10 flex items-center justify-center">
                                            ...
                                            <svg version="1.1" id="Layer_1"
                                                ...
                                                height="100px" viewBox="0 0 128 128"
                                                style="enable-background:new 0 0 128 128;"
                                                xml:space="preserve">
                                                <g>
                                                    <circle style="fill:#FFC200;" cx="64"
                                                        cy="64" r="64" />
                                                    <path style="fill:#F2B800;"
                                                        d="M64,0c-1.685,0-3.348,0.085-5,0.213C92.008,2.766,118,30.337,118,64s-25.992,61.232-59,63.787
                                            c1.652,0.127,3.315,0.213,5,0.213c35.348,0,64-28.652,64-64C128,28.652,99.348,0,64,0z" />
                                                    <path style="fill:#F2B800;"
                                                        d="M65,114c-25.729,0-49.093-21.93-49.999-46.928c-0.02-0.543,0.182-1.07,0.559-1.461
                                            ...
                                            z" />
                                                    <path style="fill:#F5F5F5;"
                                                        d="M112,67c0.833,23-21.16,45-47,45c-25.829,0-47.167-22-48-45H112z" />
                                                    <g>
                                                        <path style="fill:#263740;"
                                                            d="M52,42c-1.657,0-3-1.343-3-3c0-4.962-4.038-9-9-9s-9,4.038-9,9c0,1.657-1.343,3-3,3s-3-1.343-3-3
                                                c0-8.271,6.729-15,15-15s15,6.729,15,15C55,40.657,53.657,42,52,42z" />
                                                        <path style="fill:#263740;"
                                                            d="M102,42c-1.657,0-3-1.343-3-3c0-4.962-4.038-9-9-9s-9,4.038-9,9c0,1.657-1.343,3-3,3s-3-1.343-3-3
                                                c0-8.271,6.729-15,15-15s15,6.729,15,15C105,40.657,103.657,42,102,42z" />
                                                    </g>
                                                    <path style="fill:#E6E6E6;"
                                                        d="M103.512,92.42c0.95-1.268,1.83-2.576,2.637-3.92h-0.637V67h-2v21.5h-18V67h-2v21.5h-18V67h-2v21.5
                                        ...
                                        <div
                                            class="mt-5 flex items-center justify-center text-xs font-medium text-red-500 dark:text-slate-400 sm:text-xs md:text-xs lg:text-sm">
                                            <i class="far fa-exclamation-circle mr-3"></i>Terima kasih
                                            telah memberikan penilaian.
                                        </div> --}}
                                        <div
                                            class="mt-8 text-base font-medium text-red-500 dark:text-red-600">
                                            <i class="far fa-exclamation-circle"></i> Berikan penilaian
                                            Anda dibawah ini</div>
                                        <form
                                            action="{{ route('rating.store', ['id' => Auth::user()->id]) }}"
                                            method="post">
                                            @csrf
                                            ...
                                            <div class="mt-3 mb-3 flex items-center">
                                                <fieldset>
                                                    <div class="mt-4 space-y-4 flex flex-col">
                                                      <div class="flex items-center">
                                                        <input id="push-everything" type="radio" class="focus:ring-indigo-500 h-4 w-4 text-indigo-600 border-gray-300" name="star_evaluation" value="1">
                                                        <label for="push-everything" class="ml-3 block text-sm font-medium text-gray-700"> 
                                                            <div class="flex items-center">
                                                                ...
                                                                <span class="ml-3 text-sm text--gray-800 dark:text-gray-400 font-medium">Buruk Sekali</span>
                                                            </div>
                                                        </label>
                                                      </div>
                                                      <div class="flex items-center">
                                                        <input id="push-email" type="radio" class="focus:ring-indigo-500 h-4 w-4 text-indigo-600 border-gray-300" name="star_evaluation" value="2">
                                                        <label for="push-email" class="ml-3 block text-sm font-medium text-gray-700"> 
                                                            <div class="flex items-center">
                                                                <svg class="w-5 h-5 text-yellow-400" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"></path></svg>
                                                                <svg class="w-5 h-5 text-yellow-400" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"></path></svg>
                                                                <span class="ml-3 text-sm text--gray-800 dark:text-gray-400 font-medium">Buruk</span>
                                                            </div>
                                                        </label>
                                                      </div>
                                                      <div class="flex items-center">
                                                        <input id="push-nothing" type="radio" class="focus:ring-indigo-500 h-4 w-4 text-indigo-600 border-gray-300"  name="star_evaluation" value="3">
                                                        <label for="push-nothing" class="ml-3 block text-sm font-medium text-gray-700">
                                                            <div class="flex items-center">
                                                                <svg class="w-5 h-5 text-yellow-400" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"></path></svg>
                                                                <svg class="w-5 h-5 text-yellow-400" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"></path></svg>
                                                                <svg class="w-5 h-5 text-yellow-400" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"></path></svg>
                                                                <span class="ml-3 text-sm text--gray-800 dark:text-gray-400 font-medium">Cukup</span>
                                                            </div>
                                                        </label>
                                                      </div>
                                                      <div class="flex items-center">
                                                        <input id="push-nothing" type="radio" class="focus:ring-indigo-500 h-4 w-4 text-indigo-600 border-gray-300" name="star_evaluation" value="4">
                                                        <label for="push-nothing" class="ml-3 block text-sm font-medium text-gray-700">
                                                            <div class="flex items-center">
                                                                <svg class="w-5 h-5 text-yellow-400" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"></path></svg>
                                                                <svg class="w-5 h-5 text-yellow-400" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"></path></svg>
                                                                <svg class="w-5 h-5 text-yellow-400" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"></path></svg>
                                                                <svg class="w-5 h-5 text-yellow-400" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"></path></svg>
                                                                <span class="ml-3 text-sm text--gray-800 dark:text-gray-400 font-medium">Baik</span>
                                                            </div>
                                                        </label>
                                                      </div>
                                                      <div class="flex items-center">
                                                        <input id="push-nothing" type="radio" class="focus:ring-indigo-500 h-4 w-4 text-indigo-600 border-gray-300" name="star_evaluation" value="5">
                                                        <label for="push-nothing" class="ml-3 block text-sm font-medium text-gray-700">
                                                            <div class="flex items-center">
                                                                <svg class="w-5 h-5 text-yellow-400" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"></path></svg>
                                                                <svg class="w-5 h-5 text-yellow-400" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"></path></svg>
                                                                ...
                                                                <svg class="w-5 h-5 text-yellow-400" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"></path></svg>
                                                                <svg class="w-5 h-5 text-yellow-400" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"></path></svg>
                                                                <span class="ml-3 text-sm text--gray-800 dark:text-gray-400 font-medium">Baik Sekali</span>
                                                            </div>
                                                        </label>
                                                      </div>
                                                    </div>
                                                ...
                                                {{-- <div class="rating mb-5">
                                                    <input type="radio" id="rating-5"
                                                        value="5" name="star_evaluation">
                                                    <label for="rating-5"></label>
                                                    <input type="radio" id="rating-4"
                                                        value="4" name="star_evaluation">
                                                    <label for="rating-4"></label>
                                                    <input type="radio" id="rating-3"
                                                        value="3" name="star_evaluation">
                                                    <label for="rating-3"></label>
                                                    <input type="radio" id="rating-2"
                                                        value="2" name="star_evaluation">
                                                    <label for="rating-2"></label>
                                                    <input type="radio" id="rating-1"
                                                        value="1" name="star_evaluation">
                                            ...
                                            <div class="relative">
                                                <div
                                                    ...
                                                    <div
                                                        class="rounded-t-lg bg-white py-2 px-4 dark:bg-gray-800">
                                                        <label for="reason" class="sr-only"></label>
                                                        <textarea id="reason" name="reason" rows="4"
                                                            class="@error('reason') is-invalid @enderror w-full border-0 bg-white px-0 text-sm text-gray-900 focus:ring-0 dark:bg-gray-800 dark:text-white dark:placeholder-gray-400""
                                                            placeholder="Tulis alasan disini" required></textarea>
                                                    </div>
                                                    <div
                                                        class="flex items-center justify-between border-t py-2 px-3 dark:border-gray-600">
                                                        <button type="submit"
                                                            class="inline-flex items-center rounded-lg bg-blue-700 py-2.5 px-4 text-center text-xs font-medium text-white hover:bg-blue-800 focus:ring-4 focus:ring-blue-200 dark:bg-gray-800 dark:focus:ring-blue-900">
                                                            Kirim penilaian
                                                        </button>
                                                    </div>
                                                </div>
                                    ...
                                    {{-- <div class="flex flex-wrap items-center py-3">
                                        ...
                                        <div class="text-lg font-bold">Penilaian Ditutup</div>
                                    </div>
                                    <div
                                        class="mt-5 flex items-center justify-center text-xs font-medium text-red-500 dark:text-slate-400 sm:text-xs md:text-xs lg:text-sm">
                                        <i class="far fa-exclamation-circle mr-3"></i>Anda tidak dapat
                                        memberikan penilaian pada topik ini.
                                    </div> --}}
                            </div>
                        </div>
                    @endforeach
                ...
                <div class="text-white">
                    <div class="flex flex-wrap">
                        <div class="m-auto text-center">
                            <div>
                                <svg class="h-48 w-56 md:h-48 md:w-56 lg:h-72 lg:w-96"
                                    viewBox="0 0 631 379" fill="none"
                                    xmlns="http://www.w3.org/2000/svg">
                            ...
                            <p
                                class="mb-4 p-2 text-sm text-gray-700 dark:text-yellow-300 md:text-base">
                                Maaf penilaian sedang kosong
                            </p>
                        </div>
                    </div>
                </div>
        ...
        @if (session()->has('success'))
            <div class="alert-toast fixed bottom-0 right-0 m-8 w-full max-w-md px-2 md:w-full"
                x-data="{ open: true }" x-show.transition="open">
                <input type="checkbox" class="hidden" id="footertoast">

                <label
                    class="close flex w-full cursor-pointer items-center justify-between rounded bg-yellow-500 p-2 px-2 font-medium text-white opacity-90 shadow-lg"
                    title="close" for="footertoast">
                    <span class="mr-2 ml-6 inline-block align-middle text-xl">
                        <i class="fas fa-bell"></i>
                    </span>
                    <span class="px-auto mr-10 inline-block pr-6 align-middle text-sm">
                        {{ session('success') }}
                    </span>
                    <button
                        class="right-0 top-0 mr-6 inline-block bg-transparent align-middle text-2xl font-semibold leading-none outline-none focus:outline-none"
        ...
        @elseif(session()->has('error'))
            <div class="alert-toast fixed bottom-0 right-0 m-8 w-full max-w-md px-2 md:w-full"
                ...
                <label
                    class="close flex w-full cursor-pointer items-center justify-between rounded bg-red-600 p-2 px-2 font-medium text-white opacity-90 shadow-lg"
                    title="close" for="footertoast">
                    <span class="mr-2 ml-6 inline-block align-middle text-xl">
