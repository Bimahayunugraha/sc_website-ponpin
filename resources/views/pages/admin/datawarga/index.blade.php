                ...
                <table class="w-full whitespace-no-wrap">
                    <thead>
                        <tr
                            ...
                            <th class="px-4 py-3 uppercase">Nomor Telepon</th>
                            <th class="px-4 py-3 uppercase">Alamat</th>
                            <th class="px-4 py-3 uppercase">Role</th>
                    ...
                    <tbody class="bg-white divide-y">
                        ...
                        @forelse ($users as $user)
                            <tr class="text-gray-700 dark:text-gray-400 ">
                                ...
                                <td class="px-4 py-3 text-sm">
                                    <span class="px-2 py-1 font-semibold inline-flex leading-tight text-yellow-700 bg-indigo-100 rounded-full">
                                        {{ $user->roles }}
                                    </span>
                                </td>     
                                <td class="px-4 py-3">
                                    <div class="flex items-center space-x-4 text-sm">
                                        <a href="{{ route('datawarga.show', $user->id) }}"
                                            class="bg-green-400 hover:bg-green-700 focus:bg-green-700 text-white rounded-lg 
                                            px-3 py-3 font-semibold"><i class="far fa-info-circle"></i></a>
                                        <a href="{{ route('datawarga.edit', $user->id) }}" class="bg-sky-400 hover:bg-sky-700 focus:bg-sky-700 text-white rounded-lg px-3 py-3 font-semibold"><i class="far fa-edit"></i></a>
                                        <form onsubmit="return confirm('Yakin data {{ $user->name }} ingin dihapus?')" action="{{ route('datawarga.destroy',['id'=>$user->id])}}" method="POST">
                                            @method('delete')
                                            @csrf
                                            <button type="submit" 
